...
#define M_HALF (256 / 2)           // предполагаем, что M чётное
...
void Konstruktor::Generate_sosed_for_TVD(int* s1, int* s2, int* s12, int* s13, int* s14, int* s22, int* s23, int* s24)
{
	int km = -1;
...
// Линейная интерполяция массива arr длины M по углу phi.
// Вне диапазона — берём крайнее значение.
double interp_phi(const double* phi_center_arr, const double* arr, double phi, const int& M)
{
	...
	// Бинарный поиск: ищем lo такое, что
...
void Konstruktor::filling(void)
{
	...
	// Надо считать граничные условия из программы middle wind
	const int M = 256;
	...
	for (int j = 0; j < M; j++)
	{
		bfin2.read((char*)&phi_dummy, sizeof(double)); // phi — пропускаем
	...
	// Массив центров ячеек по phi — удобно посчитать один раз
...
{
	double V_E = phi_0;
	double ro_E_0 = 1.0 / (phi_0 * phi_0 * ae1 * ae1); // MM / (4.0 * pi * V_E * rr_0 * rr_0);
	double P_E = ro_E_0 * V_E * V_E / (ggg * M_0 * M_0);   // Мах другой в давлении
	...
	for (auto& i : this->all_Kyb)
	{
		...
		{
			double r_2 = sqrt(i->x * i->x + i->y * i->y + i->z * i->z);
			double the2 = acos(i->z / r_2);
			V_E = phi_0 * (1.0 + 0.2 * std::cos(2.0 * the2));
			double ro_E = ro_E_0 * (1.0 - 0.1 * std::cos(2.0 * the2));
